...
type DetailEventFormContentProps = Omit<DetailEventFormProps, 'visible'>;

const categoryOptions = Object.values(LocationCategoryAppearances);

export function DetailEventForm({ visible, ...contentProps }: DetailEventFormProps) {
  return (
    <Portal>
      <Dialog visible={visible} onDismiss={contentProps.onCancel}>
        <DetailEventFormContent {...contentProps} />
      </Dialog>
    </Portal>
  );
}

export function DetailEventFormContent({ draft, onCancel, onSave, savedLocations }: DetailEventFormContentProps) {
